...
export function EntrepreneurShell({
  user,
  activeSection,
  children,
  onSignOut,
  ...
  user: AuthUser;
  activeSection: EntrepreneurSection;
  children: ReactNode;
  onSignOut: () => void;
      ...
      <div className="grid min-h-screen lg:grid-cols-[268px_minmax(0,1fr)]">
        ...
        >
          ...
          <nav
            ...
            aria-label="Entrepreneur dashboard navigation"
          >
            {navigation.map(({ section, label, icon: Icon, badge }) => {
              ...
              return (
                <Link
                  key={section}
                  href={entrepreneurSectionPath(section)}
                  onClick={() => setSidebarOpen(false)}
          ...
          <div className="space-y-3 border-t border-slate-100 p-4">
            <div className="rounded-2xl border border-emerald-100 bg-gradient-to-br from-emerald-50 via-white to-white p-4">
              ...
              <p className="mt-3 text-sm font-bold text-slate-800">Raise capital faster</p>
              <p className="mt-1 text-xs leading-5 text-slate-500">
                Keep your milestones and valuation data updated to attract verified investors.
              </p>
              <Link
                href="/dashboard/entrepreneur/campaigns"
                className="mt-4 inline-flex items-center gap-1 text-xs font-bold text-emerald-700"
              >
                Manage campaigns <span aria-hidden="true">→</span>
              </Link>
            </div>
            <Link
              href="/faq"
                ...
                >
                  ...
                  <span className="hidden text-left leading-tight md:block">
                    <span className="block text-sm font-bold text-slate-800">
                      {user.firstName || "Founder"} {user.lastName || ""}
                    </span>
                    <span className="block text-xs text-slate-500">Verified founder</span>
                  </span>
                  <ChevronDown
