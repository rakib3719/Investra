  ...
  return (
    <main className="min-h-screen bg-slate-50 flex items-center justify-center p-6">
      <section className="max-w-md w-full bg-white rounded-3xl border border-slate-200 shadow-xl p-8 space-y-5 text-center">
        ...
        {token && verify.isPending && <><h1 className="text-2xl font-black">Verifying your email…</h1><p className="text-sm text-slate-500">Please wait a moment.</p></>}
        ...
        {(!token || verifyError) && <form onSubmit={handleSubmit(onResend)} className="space-y-3 text-left"><label className="text-xs font-bold text-slate-700" htmlFor="email">Need another link?</label><input id="email" type="email" className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm" {...register('email')} />{errors.email && <p className="text-xs text-red-600">{errors.email.message}</p>}{resendError && <p role="alert" className="text-xs text-red-600">{resendError}</p>}{resend.isSuccess && <p className="text-xs text-emerald-700">If needed, a new verification email has been sent.</p>}<button type="submit" disabled={resend.isPending} className="w-full border border-[#064e3b] text-[#064e3b] py-3 rounded-xl text-sm font-bold disabled:opacity-60">{resend.isPending ? 'Sending…' : 'Resend verification email'}</button></form>}
      </section>
    </main>
  );
}
